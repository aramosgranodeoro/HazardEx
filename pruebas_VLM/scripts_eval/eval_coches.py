"""
evaluar_modelos_ccd.py
------------------------
Igual que evaluar_modelos_weapons.py (binario, con precision/recall/f1/mcc/auc)
pero adaptado a la categoría "accidente" (dataset CCD):

  - LABEL_MAP: crash/accidente -> accidente ; no_crash/no_accidente -> no_accidente
  - Inválidos: 'ERROR' y 'unknown' (Salamandra los usa cuando no decide;
    en este dataset son más de la mitad de las predicciones).

Uso:
  python evaluar_modelos_ccd.py --input <carpeta_csvs> --output <carpeta_resultados>
"""
...
# Mapa de normalización de etiquetas para la categoría "accidente"
LABEL_MAP = {
    "crash":         "accidente",
    "accidente":     "accidente",
    "no_crash":      "no_accidente",
    "no-crash":      "no_accidente",
    "no crash":      "no_accidente",
    "non_crash":     "no_accidente",
    "no_accidente":  "no_accidente",
    "no-accidente":  "no_accidente",
    "no accidente":  "no_accidente",
}

# Valores de predicción considerados inválidos (case-insensitive), además del
# --invalid_label indicado por argumento.
EXTRA_INVALID_VALUES = {"UNKNOWN"}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evalúa modelos de clasificación de accidentes (CCD) a partir de CSVs de resultados."
    )
    parser.add_argument("--input",  "-i", required=True,
                        help="Carpeta con los CSVs de resultados.")
    parser.add_argument("--output", "-o", default="resultados_evaluacion_ccd",
                        help="Carpeta raíz de salida (default: resultados_evaluacion_ccd).")
...
def clean_df(df, col_real, col_pred, col_conf, invalid_label):
    ...
    invalid_set = EXTRA_INVALID_VALUES | {invalid_label.upper()}
    is_invalid = df[col_pred].astype(str).str.upper().isin(invalid_set)
...
def plot_classification_metrics(df_summary, output_folder):
    ...
    ax.set_title("Comparación de métricas de clasificación por modelo (accidentes)",
...
def plot_cost_metrics(df_summary, output_folder):
    ...
    fig.suptitle("Comparación de métricas de coste computacional (media por muestra) — accidentes",
...
def main() -> None:
    ...
    for csv_path in csv_files:
        ...
        print(f"  Predicciones inválidas ('{args.invalid_label}'/UNKNOWN): {n_inv} ({pct_inv}%)")
        ...
        report_path.write_text(
            ...
            f"Predicciones inválidas ('{args.invalid_label}'/UNKNOWN): {n_inv} ({pct_inv}%)\n\n"
            + report_str,
            encoding="utf-8",
        )
