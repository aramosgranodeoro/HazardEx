...
def analyze_chart(pil_image, query, model):
    ...
    response = ollama.chat(
        ...
        messages=[{
            ...
            'content': f"""
            # ROL
            Eres un analista de gráficos. No pienses en voz alta. No uses etiquetas <tool_call>. Responde ÚNICAMENTE con el JSON solicitado.

            # OBJETIVO
            Responde la siguiente pregunta sobre la imagen del gráfico.
...
if __name__ == "__main__":
    ...
    for model in modelos:
        ...
        analizar_dataset(output_csv=f"resultados_chartsQA_{model_name_safe}_es.csv", model=model)