...
def analizar_imagen(pil_image, model="llava:7b"):
    response = ollama.chat(
        ...
        messages=[{
            "role": "system",
            "content": "Eres un analista de seguridad. No pienses en voz alta. Responde ÚNICAMENTE con el JSON solicitado."
        }, {
            "role": "user",
            "content": """
            # ROL
            Eres un analista de seguridad revisando imágenes.

            # OBJETIVO
            Determina si la imagen contiene un arma peligrosa (cuchillo o pistola) o no.

            # DIRECTRICES
            - Responde "arma" si ves un cuchillo, una pistola o cualquier cosa que parezca un arma (navaja, escopeta, etc.).
            - Responde "no_arma" si no ves ningún objeto que parezca un arma.

            # FORMATO DE SALIDA
            Responde ÚNICAMENTE con JSON válido:
            {
                "categoria_predicha": "arma | no_arma",
                "confianza": 0.0,
...
def parsear_respuesta(raw):
    ...
    try:
        ...
        match = re.search(r'"categoria_predicha"\s*:\s*"([^"]+)"', raw, re.IGNORECASE)
        if match:
...
if __name__ == "__main__":
    ...
    for model in modelos:
        ...
        analizar_dataset(output_csv=f"resultados_weapons_{model_name_safe}_es.csv", model=model)